...
public class ByteArray {
	...
	private static Object readObject(InputStream is) throws IOException {
		...
		int type = is.read();
		switch (type) {
			case ByteArray.TypeNull:
				value = null;
				break;
			case ByteArray.TypeString:
				int length = readInt(is);
				byte[] string = new byte[length];
				is.read(string);
				value = toString(string);
				break;
			case ByteArray.TypeIntegral:
				value = readLong(is);
				break;
			case ByteArray.TypeFloating:
				value = readDouble(is);
				break;
			case ByteArray.TypeDate:
				value = new Date(readLong(is));
				break;
			case ByteArray.TypeList:
				value = readList(is);
				break;
			case ByteArray.TypeMap:
				value = readMap(is);
				break;
			case ByteArray.TypeBinary:
				int len = readInt(is);
